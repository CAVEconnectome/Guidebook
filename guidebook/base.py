...
def process_skeleton_from_mesh(
    mesh, root_loc=None, root_is_soma=False, close_mean=10000, sk_kwargs={}
):
    if root_is_soma:
        soma_loc = root_loc
    else:
        ...
        for cc in modest_ccs:
            if (
                np.mean(np.linalg.norm(
                    sk.vertices[lbls == cc] - soma_loc, axis=1))
                < close_mean
            ):
...
def base_sb_data(
    client, oid, focus_loc=None, fixed_ids=[], black=0.35, white=0.7, view_kws={}
):
...
def end_point_sb_data(client, oid, skf, labels, tags=[], active=False, color="#FFFFFF"):
...
def proofer_statebuilder(
    ...
    state_server = client.state.state_service_endpoint
    img = sb.ImageLayerConfig(
        client.info.image_source(), contrast_controls=True, black=0.35, white=0.7
    )
    seg = sb.SegmentationLayerConfig(
        client.info.segmentation_source(),
        fixed_ids=[oid],
    ...
    sb_ep = sb.StateBuilder(
        layers=[img, seg, ep_layer], state_server=state_server, view_kws=view_kws
    )
    sb_bp = sb.StateBuilder(layers=[bp_layer], view_kws={"layout": "3d"})
...
def process_points_from_skeleton(
    oid,
    sk,
    client,
    cp_percentile=98,
    ...
    sb_dfs = (
        ep_df.sort_values(by=["ep_group", "dfr"]),
        bp_df.sort_values(by=["bp_group", "dfr"]),
    )
    pf_sb = proofer_statebuilder(
        oid,
        client,
        sk.vertices[sk.root] / np.array([4, 4, 40]),
        bp_proofreading_tags,
        ep_proofreading_tags,
    )
    return pf_sb, sb_dfs
...
def generate_proofreading_state(
    ...
    skf = process_skeleton_from_mesh(
        mesh, root_loc=root_loc, root_is_soma=root_is_soma, sk_kwargs=SK_KWARGS
    )
...
def lvl2_fragment_locs(l2_ids, cv):
...
def get_lvl2_skeleton(
    client,
    root_id,
    convert_to_nm=False,
    refine_branch_points=False,
    refine_end_points=False,
    root_point=None,
    point_radius=200,
    invalidation_d=3,
    verbose=False,
    ...
    if refine_branch_points or refine_end_points:
        if verbose:
            t4 = time.time()
        sk_ch, missing_ids = refine_skeleton(
            sk_ch,
            l2dict_reversed,
            cv,
            convert_to_nm,
            root_location=lvl2_root_loc,
            refine_branch_points=refine_branch_points,
            refine_end_points=refine_end_points,
        )
        if verbose:
...
def lvl2_statebuilder(datastack, root_id):
    ...
    img = sb.ImageLayerConfig(
        client.info.image_source(), contrast_controls=True, black=0.35, white=0.66
    )
    seg = sb.SegmentationLayerConfig(
...
def update_lvl2_skeleton(l2_sk, l2br_locs):
    ...
    return skeleton.Skeleton(
        vertices=verts, edges=l2_sk.edges, remove_zero_length_edges=False
    )
...
def refine_skeleton(
    ...
    if refine_branch_points:
        l2br_locs, bp_missing_ids = lvl2_branch_fragment_locs(
            l2_sk, l2dict_reversed, cv
        )
        bp_vinds = np.array(l2_sk.branch_points_undirected)
    else:
        l2br_locs = np.zeros((0, 3))
        bp_missing_ids = np.zeros(0, dtype=int)
        bp_vinds = np.zeros(0, dtype=int)
    if refine_end_points:
        l2ep_locs, ep_missing_ids = lvl2_end_fragment_locs(
            l2_sk, l2dict_reversed, cv)
        ep_vinds = np.array(l2_sk.end_points_undirected)
    else:
        l2ep_locs = np.zeros((0, 3))
        ep_missing_ids = np.zeros(0, dtype=int)
        ep_vinds = np.zeros(0, dtype=int)

    l2_locs = np.vstack([l2br_locs, l2ep_locs])
    missing_ids = np.concatenate([bp_missing_ids, ep_missing_ids])
    vinds = np.concatenate([bp_vinds, ep_vinds])

    missing_inds = np.any(np.isnan(l2_locs), axis=1)
    verts[vinds[~missing_inds]] = l2_locs[~missing_inds]

    if convert_to_nm:
        other_inds = np.full(len(verts), True)
        other_inds[vinds[~missing_inds]] = False
        verts[other_inds] = (
            chunk_to_nm(verts[other_inds], cv) + chunk_dims(cv) // 2
...
def generate_lvl2_proofreading(
    ...
    l2_sk, l2dict_reversed = get_lvl2_skeleton(
        client,
        root_id,
